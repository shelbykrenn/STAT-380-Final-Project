...
There are 11 animals that have names with negative scores. Negative scores indicate that the animals have names with words that have a bad sentiment.  People may not want to adpot animals that have names with negative sentiments.  Shelters may want to consider changing these names to help get these animals adpoted.  In addition they could look to give them names that have positive sentiment.  
...
PetFinderFit <- 
  Joined %>%
  mutate(fit = ifelse((Vaccinated == 'Yes') & (Dewormed == 'Yes'), 
                      TRUE, FALSE)) %>%
...
```{r}
Tally <- tally(~ Sterilized, data = PetFinderFit)
Tally
```
...
Most animals are for sure not vaccinated, dewormed, and steralized (76%).  Over 3 times as many animals do not for sure have these proceedures as have these proceedures. It may be especially useful to look at steralization because unsteralized animals may produce more animals.  Most animals are not for sure not steralized (64%).  This would make it more difficult for older animals to get adopted.  While it would cost money to perform these proceedures, it would help get the animals adpoted.  
=======
```{r}
...
>>>>>>> 90279c1944b522f09195b6f61b8f7aa16e48864f
